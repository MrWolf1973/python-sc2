...
class BoSALT_Test(sc2.BotAI):
    def __init__(self):
        self.bosalt = BoSALT()
        self.salt = "$49773|spawningtool.com||~* 1 /+ H !, K ,/!;\" /!= )/!J #0\"  !0\"%!%2\"-!%2\"/ /3\"; !3\"C!%3\"C!%6\"Q 07\"W\" 7\"[!%7\"[!%:#(!%:#(#+;#/ );#1!%;#1!%>#7 %@#A ,@#D!%E#M /K$* .K$* *[$U!&[$U!&[$Z $a%+#0"


    async def on_step(self, iteration):
        if iteration == 0:
            await self.chat_send("(gg)")
            self.bosalt.set_bo_salt(self.salt)
            await self.chat_send("Where are u from?")
        if iteration % 100 == 1:
            unit = await self.bosalt.nextbestboaction(game_data=self)
            await self.chat_send("(probe)" + str(iteration) + ":" + str(unit))
            await self.managebuild(unit)
        await self.distribute_workers()

    async def managebuild(self, unit):
        if unit == SCV:
            for cc in ((self.units(COMMANDCENTER).ready | self.units(ORBITALCOMMAND).ready  | self.units(PLANETARYFORTRESS).ready)):
